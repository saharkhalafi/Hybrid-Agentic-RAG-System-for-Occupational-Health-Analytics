...
def _gcp_available() -> bool:
    """Return True when the embedding client is available."""
...
def test_candidate_path_rejected():
    from persistence.knowledge_pipeline import (
        is_production_gold_table,
        reject_candidate_sync,
    )

    candidate = GOLD_CANDIDATES / "table_056_01.json"

    assert reject_candidate_sync(candidate) is True
    ...
    allowed, reason = is_production_gold_table(
        payload,
        candidate,
    )
...
def test_gold_allowed_gate():
    """
    Production gold tables must be explicitly allowed.

    This test intentionally verifies the production gate using an actual
    gold artifact, but does not assume that the gate should mutate the
    artifact or derive permission from unrelated fields.
    """
    from persistence.knowledge_pipeline import is_production_gold_table

    gold_files = sorted(GOLD_TABLES.glob("*.json"))

    assert gold_files, (
        f"No production gold tables found in {GOLD_TABLES}"
    )
    ...
    for path in gold_files:
        ...
        if allowed:
            valid_gold_files.append(path)

    assert valid_gold_files, (
        "No table under gold/tables is currently accepted by "
        "is_production_gold_table(). "
        "This indicates a production-gate/schema problem rather than "
        "a missing test fixture."
    )

    # Verify the accepted artifact explicitly declares production approval.
    for path in valid_gold_files:
        payload = json.loads(
            path.read_text(encoding="utf-8")
        )

        assert payload.get("gold_allowed") is True, (
            f"{path.name} was accepted by the production gate but does "
            "not declare gold_allowed=True"
        )
...
def test_numeric_integrity_original_vs_normalized():
    ...
    assert path.exists(), (
        f"Expected gold artifact does not exist: {path}"
    )
    ...
    for row in payload.get("rows", []):
        for field in ("TWA", "STEL", "STEL/C"):
            ...
            if (
                orig
                and norm
                and str(orig).strip() != str(norm).strip()
            ):
                mismatches.append(
                    (
                        field,
    ...
    assert len(mismatches) >= 3, (
        "Expected several original≠normalized OHE6 fields"
    )
...
def test_formula_deterministic_calculation():
    from knowledge.calculation import (
        evaluate_vibration_daily_exposure,
    )
...
def test_semantic_review_required_rejected_from_production():
    ...
    allowed, reason = is_production_semantic_record(
        record
    )
...
def test_semantic_accepted_for_production():
    ...
    allowed, _ = is_production_semantic_record(
        record
    )
...
def test_semantic_promotion_accepts_pending():
    ...
    from persistence.semantic_gold_promotion import (
        promote_semantic_gold_for_production,
    )

    stats = promote_semantic_gold_for_production(
    ...
    production = (
        get_settings().gold_dir
        / "rag"
        / "semantic_text_production.jsonl"
    )
...
def test_phase_b_tables_exist():
    ...
    assert not missing, (
        f"Missing Phase-B database tables: "
        f"{sorted(missing)}"
    )
...
def test_semantic_production_sync():
    ...
    with session_scope() as session:
        stats = sync_semantic_production_corpus(
            session
        )
        ...
        count = session.scalar(
            ...
            .where(
                ...
                == SEMANTIC_SOURCE_TYPE,
                DocumentChunk.validation_status
                == "accepted",
            )
        )
...
def test_formula_registry_sync():
    ...
    from database.session import session_scope
    from persistence.formula_registry import (
        sync_gold_formulas,
    )

    formula_files = list(
        GOLD_FORMULAS.glob("*.json")
    )
    ...
    with session_scope() as session:
        ...
        limit = session.scalars(
            select(OELChemicalLimit).where(
                OELChemicalLimit.source_row_key.like(
                    "table_055_01:%"
                )
            )
        ).first()
        ...
        meta = KnowledgeMetadata(
            ...
            document_id=meta_dict.get(
                "document_id",
                "",
            ),
            gold_artifact_path=(
                limit.gold_artifact_path
            ),
            source_reference=SourceReference(
                **(
                    meta_dict.get(
        ...
        twa_data = (
            limit.original_values
            or {}
        ).get("TWA") or {}

        twa_orig = twa_data.get(
            "original_value"
        )

        twa_norm = twa_data.get(
            "normalized_value"
        )
...
@pytest.mark.skipif(
    not _gcp_available(),
    reason="GCP embedding client unavailable",
)
@pytest.mark.skipif(
    not _db_available(),
    reason="PostgreSQL not available",
...
def test_gemini_persian_embedding_and_retrieval():
    ...
    persian_text = (
        "حد مجاز مواجهه شغلی برای عوامل "
        "شیمیایی در محیط کار"
    )
